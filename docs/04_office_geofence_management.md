...
## Overview
The **Office Station & Geofence Management** feature enables administrators to configure physical office locations, work sites, and field branches with precise GPS coordinates (Latitude & Longitude) and custom geofence radii (in meters). All employee check-ins are validated against these boundaries in real-time.

---

## 1. Key Functionalities

1. **Office Station Configuration**:
   - Defines Station Name (e.g., `Store - 12` or `Musaffah M12 Depot`).
   - Defines Physical Address.
   - Sets Latitude & Longitude coordinates.
   - Sets Geofence Radius in meters (e.g., `100.0m`, `200.0m`, `500.0m`).
...
3. **Geofence Distance Calculation Algorithm (Haversine Formula)**:
   - Evaluates the precise spherical distance between employee hardware GPS coordinates `(lat1, lon1)` and office target coordinates `(lat2, lon2)` using the Haversine formula:
...
| [`lib/features/admin/presentation/admin_cubit.dart`](file:///c:/Users/srirs/.gemini/antigravity-ide/scratch/attendance_app/lib/features/admin/presentation/admin_cubit.dart) | Cubit managing office station CRUD operations in local Hive storage & Supabase cloud tables. |
