...
| **05** | [Attendance Workflow & Camera Verification Feature](05_attendance_workflow_and_camera.md) | Strict 4-step daily duty cycle (`1. Office Check-In` ➔ `2. Site Check-In` ➔ `3. Site Check-Out` ➔ `4. Office Check-Out` ➔ `Shift Completed`), `SiteNameDialog` dropdown selection (`RELAAM`, `CARRIER`, `MOPA`, `MPM`, `ELV`, `OTHERS`), live camera selfie capture, 480px JPEG image compression engine (99% payload reduction), and timeline time badges. |
...
| **07** | [Admin Dashboard, Live Tracking & Analytics Feature](07_admin_dashboard_and_analytics.md) | Executive KPI cards, real-time OpenStreetMap live employee tracking map, geofence radius visual rings, attendance feeds, and Standalone Enterprise Web Admin Portal (`web_admin/`). |
| **08** | [Reports & Multi-Format Data Export Feature](08_reports_and_data_export.md) | Multi-criteria filter engine, CSV data generation, Excel (.xlsx) spreadsheet creation, and multi-page PDF document rendering & printing services. |
